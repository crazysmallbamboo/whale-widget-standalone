...
- 拖动窗口移动，点 ✕ 或按 `Esc` 关闭
- **右键菜单**：
  - **大小**：小 / 默认 / 大三档缩放（图片 + 字体），改大小后**窗口停在原地**不会跳走
  - **更换图片**：选择任意 PNG/JPG/GIF 图片替换鲸鱼娘
  - **恢复默认图片**：换回自带鲸鱼娘素材
  - **设置 API Key**：手动填入 DeepSeek API Key（无需 DSH）
  - **立即刷新** / **回到右下角** / **关闭**

大小、图片、API Key **以及窗口位置**都会**自动保存**到 `%APPDATA%\whale-widget\config.json`，下次打开沿用。
...
python test_gui_regressions.py  # 25 项 GUI 交互回归（位置保持/恢复、失败保留数据、静默失败可报告）
